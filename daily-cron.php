...
if( $argv[1]=='bluesky' )
{
    ...
    php_logln("[Bluesky] QOTD SUCCESS:\n$qotd");
    exit(0);
}
...
if( $argv[1]=='mastodon' )
{
    ...
    // check the toot url
    if(! isset( $activityPubJson['url'] ) )
        mstdn_die("incomplete toot response", $res);
    // must match the URL with the
    if( $activityPubJson['url'] != $res['url'] )
        mstdn_die("invalid toot url", $res);

    php_logln("[Mastodon] QOTD SUCCESS:\n$qotd");
    exit(0);
}
